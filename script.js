...
// NEW: passenger presence tracking
...
async function fetchVehicles(){
  try {
    ...
    const livePassengerIds = new Set();

    vehiclesData.forEach(v=>{
      if (!v.lat || !v.lon) return;
      if (v.role.toLowerCase().includes("passenger")) {
        livePassengerIds.add(v.id);
        const popup = `🧍 Passenger at <b>${v.stop_name || "waiting"}</b>${v.route_id ? `<br>Route: ${v.route_id}` : ""}`;
        if (passengerMarkers[v.id]){
          passengerMarkers[v.id].setLatLng([v.lat,v.lon]).setPopupContent(popup);
        } else {
          passengerMarkers[v.id] = L.marker([v.lat,v.lon], {icon: passengerIcon}).bindPopup(popup).addTo(map);
        }
      } else {
    ...
    // Remove passengers no longer sharing
    Object.keys(passengerMarkers).forEach(id=>{
      if (!livePassengerIds.has(id)){
...
// ================== PASSENGER SIDEBAR (NEW) ==================
function updatePassengerSidebar(){
  const el = $id("passengerSidebar");
  el.innerHTML = "";

  const passengersAtStop = vehiclesData.filter(v => v.role.toLowerCase().includes("passenger") && v.stop_name);
  if (passengersAtStop.length === 0){
    el.innerHTML = "<p>No passengers currently sharing location</p>";
    return;
  }
  ...
  passengersAtStop.forEach(p=>{
    if (!stopMap[p.stop_name]) stopMap[p.stop_name] = [];
    stopMap[p.stop_name].push(p.id);
  });

  for (const stop in stopMap){
    el.innerHTML += `<div>🧍 <b>${stop}</b>: ${stopMap[stop].join(", ")}</div>`;
  }
}
...
function maybeStartPassengerPresence(){
  ...
  if (!role.includes("passenger") || !selectedStopCoords || !selectedRouteId){
    stopPassengerPresence();
    return;
  }
...
function stopPassengerPresence(){
  ...
  if (passengerId){
    fetch(`${BACKEND_URL}/api/remove_vehicle`, {
      method: "POST",
      headers: {"Content-Type":"application/json"},
...
document.addEventListener("DOMContentLoaded",()=>{
  ...
  clearBtn.addEventListener("click",()=>{
    ...
    $id("passengerSidebar").innerHTML = "<p>No passengers currently sharing location</p>";
    hideBanner();
    stopPassengerPresence();
