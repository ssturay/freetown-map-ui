...
async function fetchVehiclesAndPassengers(){
  try {
    ...
    Object.keys(grouped).forEach(stop=>{
      passengerListEl.innerHTML += `<div><b>${stop}</b>: ${grouped[stop].join(", ")}</div>`;
    });
...
function hideBanner(){ const banner=$id("statusBanner"); banner.style.display="none"; banner.classList.remove("pulse-green","shake-orange"); }
...
function stopPassengerPresence(){
  ...
  if(passengerId){
    fetch(`${BACKEND_URL}/api/remove_vehicle`,{method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({id:passengerId})}).catch(()=>{});
  }
}
